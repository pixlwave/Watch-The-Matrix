...
class UserTests: BaseTestCase {
    func testConstraints() {
        func testUsers() {
            ...
            _ = dataController.createUser(id: "@user:example.org", in: roomA)
            _ = dataController.createUser(id: "@user:example.org", in: roomB)
            dataController.save()
            
            XCTAssertEqual(dataController.count(for: User.fetchRequest()), 2, "There should be 2 users in total")
            ...
            _ = dataController.createUser(id: "@user2:example.org", in: roomB)
            dataController.save()
            
            // then this user should be created
            XCTAssertEqual(dataController.count(for: User.fetchRequest()), 3, "There should be 3 users in total")
        }
    }
}
