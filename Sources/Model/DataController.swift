...
class DataController {
    ...
    /// A context from the container for background operations.
    /// The context is created on the queue that first requests it.
    ...
    /// Checks for any unsaved changes in the background context, and saves them if there are.
    func backgroundSave() {
    ...
    /// This operations occurs on the background context.
    ...
    ///
    /// The request is executed on the background context.
    ...
    /// This operation is executed on the background context.
    func deleteAll() {
        ...
        backgroundSave()      // save here to ensure all data is completely deleted
    }
    
    
    // MARK: Create Objects
    /// Creates a new room with the specified ID from a Matrix `JoinedRoom`.
    /// The room is created on the background context.
    func createRoom(id: String, joinedRoom: JoinedRoom) -> Room {
        let room = Room(context: backgroundContext)
        room.id = id
    ...
    /// when it's content is received from the server. The message is created on the background context.
    ///
    ...
    ///
    /// The message is created on the background context and will be added to the room.
    func createMessage(roomEvent: RoomEvent, in room: Room) -> Message? {
        ...
        message.sender = member(id: roomEvent.sender, in: room, context: backgroundContext) ?? createMember(id: roomEvent.sender, in: room)
        message.room = room
    ...
    /// when their properties are received from the server. The member is created on the background context.
    func createMember(id: String, in room: Room) -> Member {
        let member = Member(context: backgroundContext)
        member.id = id
        member.room = room
        return member
    }
    ...
    ///
    /// The member is created on the background context.
    ...
    ///
    /// The reaction is created on the background context.
    func createReaction(roomEvent: RoomEvent, in room: Room) {
        ...
        reaction.message = message(id: messageID, context: backgroundContext) ?? createMessage(id: messageID)
        reaction.sender = member(id: roomEvent.sender, in: room, context: backgroundContext) ?? createMember(id: roomEvent.sender, in: room)
    ...
    ///
    /// The message edit is created on the background context.
    func createEdit(roomEvent: RoomEvent) {
        ...
        let edit = Edit(context: backgroundContext)
        edit.body = body
        edit.id = roomEvent.eventID
        edit.date = roomEvent.date
        edit.message = message(id: messageID, context: backgroundContext) ?? createMessage(id: messageID)
    }
    ...
    ///
    /// The redaction is created on the background context.
    func createRedaction(roomEvent: RoomEvent, in room: Room) {
        guard let messageID = roomEvent.redacts else { return }
        
        let redaction = Redaction(context: backgroundContext)
        redaction.id = roomEvent.eventID
        redaction.date = roomEvent.date
        redaction.sender = member(id: roomEvent.sender, in: room, context: backgroundContext) ?? createMember(id: roomEvent.sender, in: room)
        redaction.message = message(id: messageID, context: backgroundContext) ?? createMessage(id: messageID)
    }
    ...
    func processStateEvent(_ event: RoomEvent, in room: Room) {
        if event.type == "m.room.name", let name = event.content.name {
            ...
            if let userID = event.stateKey, let membership = event.content.membership {
                if membership == .join {
                    let member = self.member(id: userID, in: room, context: backgroundContext) ?? createMember(id: userID, in: room)
                    updateMember(member, from: event)
                } else {
                    if let member = self.member(id: userID, in: room, context: backgroundContext) {
    ...
    func room(id: String, context: NSManagedObjectContext) -> Room? {
    ...
    func message(id: String, context: NSManagedObjectContext) -> Message? {
    ...
    func member(id: String, in room: Room, context: NSManagedObjectContext) -> Member? {
